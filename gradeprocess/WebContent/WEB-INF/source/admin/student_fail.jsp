...
$(document).ready(function(){
	...
	$(".btn_info").on("click", function(){
		...
		$(".hidden_couId").val($(this).parents("tr").children().eq(0).text());
		$(".hidden_couName").val($(this).parents("tr").children().eq(1).text());
		$(".hidden_couStart").val($(this).parents("tr").children().eq(4).text());
		$(".hidden_couEnd").val($(this).parents("tr").children().eq(5).text());
	});
 	// 검색
                  ...
                  <div class="x_content">
					...
					<c:forEach var="c" items="${list}">
						<tr>
							...
							<td>${c.cou_name}</td>
							<td>${c.class_total}</td>
							<td>${c.mid_fail_count}</td>
							<td>${c.cou_start_date}</td>
							<td>${c.cou_end_date}</td>
							<td>${c.class_name}</td>
							<td>
							<form action="student_fail2.do" method="post">
								<div class="btn-group">
									<button type="submit" class="btn btn-default btn-xs btn_info">조회</button>
									<input type="hidden" class="hidden_couId" name="hidden_couId">
                  					<input type="hidden" class="hidden_couName" name="hidden_couName">
                  					<input type="hidden" class="hidden_couStart" name="hidden_couStart">
                  					<input type="hidden" class="hidden_couEnd" name="hidden_couEnd">
								</div>
							</form>
							</td>
						</tr>
