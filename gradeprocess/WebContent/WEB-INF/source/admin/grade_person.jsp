...
$(document).ready(function() {
	...
	// 조회 시 다른 페이지로 자료 넘기기
	$("button.btn_info").on("click", function() {
		$(".titleId").val($(this).parents("tr").children().eq(0).text());
		$(".titleName").val($(this).parents("tr").children().eq(1).text());
		$(".titlePhone").val($(this).parents("tr").children().eq(2).text());
	});
                  ...
                  <div class="x_content">
					...
					<c:forEach var="g" items="${list}">
						<tr>
							...
							<td>${g.st_phone}</td>
							<td>${g.cou_count}</td>
							<td><form action="grade_person2.do" method="post" class="hidden_form">
								<div class="btn-group">
								<button type="submit" class="btn btn-default btn-xs btn_info">조회</button>
								</div>
								<input type="hidden" class="titleId" name="titleId">
								<input type="hidden" class="titleName" name="titleName">
								<input type="hidden" class="titlePhone" name="titlePhone">
								</form></td>
						</tr>
					</c:forEach>
						...
						<tr>
							...
							<td>
								<div class="btn-group">
										<a href="grade_person2.do" class="btn btn-default btn-xs">조회</a>
								</div>
							</td>
						</tr>
						-->
